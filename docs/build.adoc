...
The release is done by a PowerShell script, which uses the GitHub API to do the release. By doing it like this Taskfile performs the execution of the script, so it is not done by a propriatry task in the CI/CD build pipeline.
...
The main build pipeline file has a stage called Release, which downloads the necessary artifacts and then calls the `release` task on Taskfile to perform the release.

The script will check to see if any of the required values are missing, if they are a message similar to the following is displayed.

.Taskfile release with missing required values
image::images/taskfile-release-error.png[]
...
| `azuredevops-taskfile.yml` | Main pipeline file that AzDo uses to run the build
...
. Upload manual
. Run integration tests by running the test:integration task of the TaskFile
. For releases, run release task of TaskFile
