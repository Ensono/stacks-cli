...
.NET understands the properties `latestPatch` and `latestFeature` which allows some flexibility when evaluating the version of .NET to being used. As the CLI is meant to provide quick responses to the user about the environment, it now understands this two values of `rollForward` as well.
====
