...
- name: my-webapi
  ...
  settingsfile: "C:\\Users\\RussellSeymour\\workspaces\\stacks\\stacks-cli\\local\\stackscli.yml"
  cloud:
    region: ukwest
...
It is possible to change this behaviour, by adding the `--force` option to the command line. This will remove any existing directory and recreate the project in is place.
