...
[options="header",cols="1,1,1a,1"]
|===
| Parameter | Description | Example | Allowed Values 
| `folders` | A list of folders that can be removed during scaffolding. This is useful for mono-repo based projects where multiple projects may exist but are not always required | 
[source,yaml]
----
folders:
  - ingest
...
If `folders` is specified then the list of folders in the settings file will be compared against the list supplied by the CLI. Any that do not exist in both will be removed.| `cmd` | `cmd`, `copy`, `folders`
...
==== Joining an array in template

As can be seen, a lot of substitutions can be made during the template phase. However sometimes it is necessary to transform things into a string, an array (or slice) for example.

When a scaffold file is configured for a project that has folders, a folders option is passed. This passed the template as an array, but because the `args` option expects to be a string an array will not be transformed correctly.

So to get around this issue, a `StringsJoin` template function has been added. For example given a list of folders that the CLI should keep in the project:
...
And the following snippet in the `stackslci.yml` file for a project:

[source,yaml]
----
- action: folders
  args: "{{ StringsJoin .Project.Folders \",\"}}"
  description: Only keep the named folders
----
...
This can now be read by the operations of the CLI and will only keep the folders that exist in this list and the main list in the `stackscli.yml` file.