...
| `pipeline` | Restrict this operation so it only runs when the selected pipeline type matches the specified value. If empty, the operation runs for any pipeline selection. The comparison is case-insensitive. | `azdo` | `azdo`, `gha`
| `exclude` | A list of glob patterns (using `**` for recursive matching) that identify files and directories to skip when the action is `copy`. Useful for excluding pipeline files for a CI system that is not being used. |
...
| Any valid doublestar glob pattern
|===
