...
However this was changed from `AMIDOSTACKS` in version 3.x of the tool, which may mean that some configurations break. To achieve the same behaviour as before another environment variable can be set, `STACKSCLI_ENVVARPREFIX`, for example:
