...
Creates a project that can deploy the infrastructure for Azure Container Apps
...
Creates a project that can deploy the infrastructure for Azure Kubernetes Service

| `git`

| Infra ALZ Management |

https://github.com/Ensono/stacks-azure-platform-landing-zone

Creates a project that can deploy the infrastructure for an Azure Platform Landing Zone for Management

| `zip`

| Infra ALZ Connectivity (Hub/Spoke) |

https://github.com/Ensono/stacks-azure-platform-landing-zone

Creates a project that can deploy the infrastructure for an Azure Platform Landing Zone for Connectivity, using a Hub and Spoke network topology

| `zip`

| Infra ALZ Connectivity (Virtual WAN) |

https://github.com/Ensono/stacks-azure-platform-landing-zone

Creates a project that can deploy the infrastructure for an Azure Platform Landing Zone for Connectivity, using a Virtual WAN network topology
...
Creates a project that can deploy the infrastructure for an Azure Platform Landing Zone for Identity.
...
This can be one of `git`, `nuget`, `filesystem`, or `zip`. This informs the CLI how to download the component so it can be configured for the application.

| `stacks.components.dotnet_webapi.package.url` |

If the type of package is `git`, this is the URL to the Git repository. If the type is `zip`, this is the URL to the zip archive (supports Go template variables for versioned downloads).
...
The version or branch/tag of the package to download. Only applies to `nuget` and `git` respectively. For `zip` packages with GitHub release URLs containing template variables (e.g. `{{ .Project.Framework.Version }}`), specifying `latest` or leaving blank will automatically resolve the latest GitHub release tag.
