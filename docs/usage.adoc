...
The CLI will not attempt to perform a version check if it is not able to resolve the address for GitHub, in which case it assumes that it is running in an offline environment.